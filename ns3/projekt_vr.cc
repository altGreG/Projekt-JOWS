...
//     // Print flow monitor statistics and QoS-related summaries
...
//             double lossRatio = fs.txPackets > 0 ? (double)(fs.txPackets - fs.rxPackets) / fs.txPackets : 0.0;
...
//     // Jeśli użytkownik nie poda nazw, można automatycznie ustawić
//     if (xmlFileName == "results.xml")
//     {
//         xmlFileName = enableQos ?
//             "qos_enabled_results.xml" :
//             "qos_disabled_results.xml";
//     }

//     if (csvFileName == "results.csv")
//     {
//         csvFileName = enableQos ?
//             "qos_enabled_results.csv" :
//             "qos_disabled_results.csv";
//     }
...
//     if (csvFile.is_open()) {
//         csvFile << "FlowId,Type,Source,SourcePort,Destination,DestinationPort,Protocol,TxPackets,RxPackets,LossRatio,AvgDelaySeconds\n";
//         if (classifier) {
//             std::map<FlowId, FlowMonitor::FlowStats> stats = monitor->GetFlowStats();
//             for (auto const &flow : stats) {
//                 Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow(flow.first);
...
//         }
//         csvFile.close();
//         NS_LOG_UNCOND("Saved flow monitor CSV to " << csvFileName);
//     } else {
//         NS_LOG_UNCOND("Unable to open " << csvFileName << " for CSV output.");
//     }
